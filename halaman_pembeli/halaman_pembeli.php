    ...
    <body>
        ...
        <div>
            <?php
                ...
                $query = "select barang.foto, barang.nama, merk.nama as merk, barang.harga from barang inner join merk on barang.id_merk=merk.id";
            ...
            <div class="gambar">
                <?php echo "<img src='../img/".$row['foto']."' width='150' height='150'" ?>
            </div>
            <div><?php echo $row["nama"] ?></div>
            <div><?php echo $row["merk"] ?></div>
            <div><?php echo $row["harga"] ?></div>
            <div><button class="button" type="submit" onclick="beli('<?= $row['kode']?>')">Beli</button></div>
            </div>
            <?php
                    }
                }
        ...
        function beli(kode) {
            ajax({
                ...
                success: function(data) {
                    console.log(data);
                    if(data === 'OK') {
                        console.log('Berhasil menambah ke cart');
                    }
                },
                error: function(data) {
                    console.log(data);
